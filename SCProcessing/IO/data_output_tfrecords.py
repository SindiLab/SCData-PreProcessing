...
#     except:
#         logging.warn("There was some issue with one or more lines... check the outputs")

def read_and_serialize(single_cell_object, job_path):
    """
    ...
    try:
        cat = single_cell_object.obs['cluster'].cat.categories
    except:
        raise ValueError("For now let's make sure we have it clustered! ")
        
    counter = 0;
    with TFRFWriter(worker_path, categories=cat) as writer:
        for i in range(0,single_cell_object.shape[0]):
            counter +=1;
            sc_genes, d = process_line(single_cell_object[i])
            print(counter)
            # print in TF records
            writer.write_numpy(sc_genes, d.barcode, d.count_no,
                               d.genes_no, d.dset, d.cluster)

    return 'done with writing for: '
...
class TFRFWriter:
    ...
    def open(self):
        ...
        -------
        """
        opt = tf.compat.v1.python_io.TFRecordOptions(tf.compat.v1.python_io.TFRecordCompressionType.GZIP)
        self.valid = tf.compat.v1.python_io.TFRecordWriter(self.valid_filename, opt)
        self.test = tf.compat.v1.python_io.TFRecordWriter(self.test_filename, opt)
        self.train = [tf.compat.v1.python_io.TFRecordWriter(filename, opt) for filename in
